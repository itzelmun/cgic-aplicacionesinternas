...
$registros = mysql_fetch_array($consulta);
			...
			$publicaciones = $registros['produccion_cient'];
			$foto = $registros['foto'];
		
?>
<a href="admin_ptc.php" class="large button green">Cancelar...</a>

<form id="sni" name="sni" method="post" enctype="multipart/form-data" action="update_ptc.php?id=<?=$id; ?>">
  <fieldset id="usuario">
  <legend> Modificar PTC: </legend>
  <ul>
  	<li>
  		<label>N&uacute;mero de trabajador:</label>
		<input type="text" id="num_trabajador" name="num_trabajador" value="<?=$num_trabajador; ?>"/>
  	</li>
  	<li>
  		<label>Grado acad&eacute;mico:</label>
         <select id="grado" name="grado">                                   
                                    <option value="Doctorado" <? if($grado == "Doctorado" && $genero == "Hombre") echo 'selected="selected"'; ?>>Dr.</option>
                                    <option value="Doctorado" <? if($grado == "Doctorado" && $genero == "Mujer") echo 'selected="selected"'; ?>>Dra.</option>
                                    <option value="Maestría" <? if($grado == "Maestría") echo 'selected="selected"'; ?>>M.C.</option>
                                    <option value="Licenciatura" <? if($grado == "Licenciatura" && $genero == "Hombre") echo 'selected="selected"'; ?>>Lic.</option>
                                    <option value="Licenciatura" <? if($grado == "Licenciatura" && $genero == "Mujer") echo 'selected="selected"'; ?>>Licda.</option>
                                    <option value="Ingeniería" <? if($grado == "Ingeniería") echo 'selected="selected"'; ?>>Ing.</option>
                                  </select>
  	</li>
	<li>
    ...
    <li>
        ...
        <input type="text" id="tel_ext" name="tel_ext" value="<?=$tel_ext; ?>"/>
  	</li>
    <li>
    ...
    <li>
        ...
        <select id="nivelsni" name="nivelsni">                               
                                    ...
                                    <option value="1" <? if($nivelsni == "Nivel 1" || $nivelsni == 1) echo 'selected="selected"'; ?>>1</option>
                                    <option value="2" <? if($nivelsni == "Nivel 2" || $nivelsni == 2) echo 'selected="selected"'; ?>>2</option>
                                    <option value="3" <? if($nivelsni == "Nivel 3" || $nivelsni == 3) echo 'selected="selected"'; ?>>3</option>
                                    <option value="No pertenece" <? if($nivelsni == "No pertenece") echo 'selected="selected"'; ?>>No pertenece</option>
                                  </select>
  	</li>
    <li>
  		<label>A&ntilde;o de ingreso al SNI:</label>
        <input type="text" id="ingresosni" name="ingresosni" value="<?=$ingresosni; ?>"/>
  	</li>
    <li>
  		<label>A&ntilde;o de vigencia SNI:</label>
        <input type="text" id="vigenciasni" name="vigenciasni" value="<?=$vigenciasni ?>"/>
  	</li>
    <li>
  		<label>&Aacute;rea de estudio:</label>
       <select id="area" name="area" >                                   
                                    <option value="fisico" <? if($area == "fisico" || $area == "Físico Matemáticas y Ciencias de la Tierra") echo 'selected="selected"'; ?>>F&iacute;sico Matem&aacute;tico y Ciencias de la Tierra</option>
                                    <option value="biologia" <? if($area == "biologia" || $area == "Biología y Química") echo 'selected="selected"'; ?>>Biolog&iacute;a y Qu&iacute;mica</option>
                                    <option value="medicina" <? if($area == "medicina" || $area == "Medicina y Ciencias de la Salud") echo 'selected="selected"'; ?>>Medicina y Ciencias de la Salud</option>
                                    <option value="humanidades" <? if($area == "humanidades" || $area == "Humanidades y de la Conducta") echo 'selected="selected"'; ?>>Humanidades y de la Conducta</option>
                                    <option value="sociales" <? if($area == "sociales" || $area == "Sociales y Económico Administrativas") echo 'selected="selected"'; ?>>Sociales y Econ&oacute;mico Administrativas</option>
                                    <option value="biotecnologia" <? if($area == "biotecnologia" || $area == "Biotecnología y Ciencias Agropecuarias") echo 'selected="selected"'; ?>>Biotecnolog&iacute;a y Ciencias Agropecuarias</option>
                                    <option value="ingenieria" <? if($area == "ingenieria" || $area == "Ingeniería") echo 'selected="selected"'; ?>>Ingenier&iacute;a</option>
                                    <option value="sin area" <? if($area == "sin area" || $area == "Sin área") echo 'selected="selected"'; ?>>Sin &aacute;rea</option>
                                  </select>
  	</li>
    <li>
  		<label>Cuerpo acad&eacute;mico:</label>
        <input type="text" id="cuerpo_acad" name="cuerpo_acad" value="<?=$cuerpo_acad; ?>"/>
  	</li>
    <li>
  		<label>LGAC:</label>
    ...
    <li>
          ...
          <textarea id="rec_tec" name="rec_tec" cols="100" rows="5"> <?=$rec_tec; ?> </textarea>
  	</li>
    <li>
  		<label>T&eacute;cnicas de experimentaci&oacute;n:</label>
         <textarea id="tec_exp" name="tec_exp" cols="100" rows="5"> <?=$tec_exp; ?> </textarea>
  	</li>
    <li>
  		<label>Proyectos actuales:</label>
          <textarea id="proyectos_actuales" name="proyectos_actuales" cols="100" rows="5"> <?=$proyectos_actuales; ?> </textarea>
  	</li>
    <li>
  		<label>A&ntilde;o de obtenci&oacute;n del grado:</label>
        <input type="text" id="grado_ano" name="grado_ano" value="<?=$grado_ano; ?>"/>
  	</li>
    <li>
  		<label>Instituci&oacute;n de obtenci&oacute;n:</label>
        <input type="text" id="grado_instit" name="grado_instit" value="<?=$grado_instit; ?>"/>
  	</li>
    <li>
    ...
    <li>
         ...
         <select id="promep" name="promep">                                  
                                    <option value="si"  <? if($promep == "si" || $promep == "Si") echo 'selected="selected"'; ?>>Si</option>
                                    <option value="no"  <? if($promep == "no" || $promep == "No") echo 'selected="selected"'; ?>>No</option>
                                  </select>
  	</li>
    <li>
  		<label>A&ntilde;o de ingreso al PROMEP:</label>
        <input type="text" id="ingresoprom" name="ingresoprom" value="<?=$ingresoprom; ?>"/>
  	</li>
    <li>
  		<label>A&ntilde;o de vigencia PROMEP:</label>
        <input type="text" id="vigenciaprom" name="vigenciaprom" value="<?=$vigenciaprom ?>"/>
  	</li>
    <li>
  		<label>Publicaciones:</label>
        <textarea id="publicaciones" name="publicaciones" cols="100" rows="5"><?=$publicaciones; ?></textarea>
  	</li>
     <li>
  		<label>Foto:</label>
        <input type="file" name="foto" id="foto"  /> <br />
        <img src="<? echo $foto; ?>" width="150px" height="111px" />
  	</li>
	</ul>
	</fieldset>
