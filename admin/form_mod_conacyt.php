...
$consulta = mysql_query("SELECT * FROM conacytproyectos WHERE id='$id'") or die(mysql_error()."</br></br></br> Error: Ha ocurrido un error al buscar el registro</center>");

$registros = mysql_fetch_array($consulta);
	  	
			$id = $registros['id'];
			$fuente_financ = $registros['fuente_financiamiento'];
			$responsable = $registros['nombre_responsable'];
...
<form id="conacyt" name="conacyt" method="post" action="update_conacyt.php?id=<?=$id;  ?>">
  <fieldset id="usuario">
  <legend> Modificar Proyecto CONACyT: </legend>
  <ul>
  	<li>
  		<label>Fuente de Financiamiento:</label>
        <input type="text" id="fuente" name="fuente" value="<?=$fuente_financ; ?>" />
  	</li>
	<li>
    ...
    <li>
        ...
        <input type="text" id="concepto" name="concepto" value="<?=$concepto;?>" />
  	</li>
    <li>
  		<label>Año:</label>
        <input type="text" id="año" name="año" value="<?=$año; ?>" />
  	</li>
    <li>
  		<label>Monto:</label>
