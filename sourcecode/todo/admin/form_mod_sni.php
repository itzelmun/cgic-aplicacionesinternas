...
$registros = mysql_fetch_array($consulta);
			...
			$id = $registros['id'];
			$noTrabajador = $registros['noTrabajador'];
			$puesto = $registros['puesto'];
			$expediente = $registros['expediente'];
			$grado = $registros['grado'];
			$nombre = $registros['nombre'];
			$apellido = $registros['apellido'];
			$area = $registros['area'];
			$nivel = $registros['nivelsni'];
			$vigenciaInicio = $registros['vigenciaInicio'];
			$vigenciaFin = $registros['vigenciaFin'];
			$udec = $registros['udec'];	
			$mail = $registros['mail'];
			$mailAlternativo = $registros['mailAlternativo'];
...
<form id="sni" name="sni" method="post" enctype="multipart/form-data" action="update_sni.php?id=<?=$id;  ?>">
  <fieldset id="usuario">
  <legend> Modificar investigador SNI: </legend>
  <ul>
  <li>
  		<label>Número de trabajador:</label>
        <input type="text" id="noTrabajador" name="noTrabajador" value="<?=$noTrabajador; ?>" />
  	</li>
     <li>
        ...
        <select id="puesto" name="puesto">
                                    ...
                                    <option value="PTC" <? if($puesto == "PTC") echo 'selected="selected"'; ?>>PTC</option>
                                    <option value="horas" <? if($puesto == "horas") echo 'selected="selected"'; ?>>Profesor por horas</option>
                                    <option value="postdoc" <? if($puesto == "postdoc") echo 'selected="selected"'; ?>>Estancia Postdoctoral</option>
                                    <option value="catedras" <? if($puesto == "catedras") echo 'selected="selected"'; ?>>Cátedras CONACyT</option>
                                    <option value="otro" <? if($puesto == "otro") echo 'selected="selected"'; ?>>Otro</option>
                                  </select>
  	</li>
    <li>
  		<label>Expediente SNI:</label>
        <input type="text" id="expediente" name="expediente" value="<?=$expediente; ?>" />
  	</li>
  	<li>
    ...
    <li>
        ...
        <input type="text" id="vigenciaInicio" name="vigenciaInicio" value="<?=$vigenciaInicio; ?>" />
  	</li>
    <li>
  		<label>Vigencia fin:</label>
        <input type="text" id="vigenciaFin" name="vigenciaFin" value="<?=$vigenciaFin; ?>" />
  	</li>
    <li>
  		<label>U de C:</label>
    ...
    <li>
        ...
        <input type="text" id="mailAlternativo" name="mailAlternativo" value="<?=$mailAlternativo; ?>" />
  	</li>
     <li>
    ...
    <li>
        ...
        <? if($nombramiento != ""){?> <a href="<?=$nombramiento?>" target="_blank" style="text"> <img src="img/pdf.png" alt="PDF" width="20" height="25"></a><? } ?>
        <input type="file" name="nombramiento" id="nombramiento">
  	</li>
    <li>
  		<label>Comentarios:</label>
        <textarea id="comentarios" name="comentarios"><?=$comentarios; ?></textarea>
       	<input type="hidden" name="nombramientoOld" id="nombramientoOld" value="<?=$nombramiento; ?>">
  	</li>
	</ul>
	</fieldset>
